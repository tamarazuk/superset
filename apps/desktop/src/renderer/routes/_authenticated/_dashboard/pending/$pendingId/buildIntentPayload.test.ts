...
function makePending(
	...
	return {
		...
		intent: "fork",
		name: "my-workspace",
		workspaceNameWasAutoGenerated: true,
...
describe("buildForkPayload", () => {
	...
	test("propagates workspaceNameWasAutoGenerated=false for user-typed names", () => {
		const pending = makePending({ workspaceNameWasAutoGenerated: false });
		const payload = buildForkPayload("pid", pending, undefined);
		expect(payload.names.workspaceNameWasAutoGenerated).toBe(false);
	});
});
